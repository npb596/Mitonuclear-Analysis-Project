...
for d in $gene_sequence; do
    min_length=`grep ${gene_array[$d]} ../exon_experimentation/random_nuclear_exons.bed | awk '{diff = $3 - $2; summed += diff} END {print summed}'`
    echo $min_length > fdM_minimum.txt
    fdM_minimum_variable=`awk '{OFS="\t"} { minimum = $1/1000 * 5; print minimum }' fdM_minimum.txt | sed 's/\.[0-9]*//'`
