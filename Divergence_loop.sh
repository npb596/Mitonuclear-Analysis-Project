...
for d in $gene_sequence; do
    ...
    vcftools --gzvcf ../recode_experimentation/nuclear_exon_recodes/nuclear_exons.${chr_array[${d}]} --bed temp_file.bed --recode --out temp_file
    python ~/genomics_general/VCF_processing/parseVCF.py -i temp_file.recode.vcf -o temp_file.geno --skipIndels --skipMono 
    python ~/genomics_general/popgenWindows.py -g temp_file.geno -o divergence_${gene_array[$d]} -f phased -w ${length_array[$d]} -m ${minimum_variable} -p Sinica Tibetan-macaque-NO.3,XH1.Assamensis,A20 -p Fascicularis BGI-96346.mulatta,BGI-CE-4.fascicularis,BGI.Mulatta-1,CR-5.Mulatta-2 -p Arctoides Malaya,SM1.Arctoides-1,SM2.Arctoides-2 -p Silenus PM664.nemestrina,PF660.nigra,PM592.tonkeana -T 4
done
...
grep "scaffold" divergence_ZNF217 > Divergence_matching_nuclear_measurements.csv
cat divergence* > divergence_measurements_temp
grep -v "scaffold" divergence_measurements_temp >> Divergence_matching_nuclear_measurements.csv
